{<img src="https://travis-ci.org/duckworth/vestal_versions.svg?branch=master" alt="Build Status" />}[https://travis-ci.org/duckworth/vestal_versions]
= Vestal Versions
...
This fork adds the ability to find versions where a specific attribute changed, useful for determining the previous value of a tracked attribute. The 2 new methods are <tt>versions.find_with_attribute(attr)</tt> and <tt>versions.find_last_with_attribute(attr)</tt>. (See example below for usage.)

It also namespaces the versions table to "vestal_versions" and includes compatability for Rails 4+ with ProtectedAttributes gem.
